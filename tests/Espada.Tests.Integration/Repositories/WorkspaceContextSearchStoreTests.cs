...
namespace Espada.Tests.Integration.Repositories
{
    ...
    public sealed class WorkspaceContextSearchStoreTests(PostgreSqlDatabaseFixture fixture)
        : PostgreSqlIntegrationTest(fixture)
    {
        ...
        public async Task Search_ShouldReturnHybridScoresAndApplyPriorityFilter()
        {
            ...
            using (IServiceScope scope = serviceProvider.CreateScope())
            {
                ...
                await scope.ServiceProvider.GetRequiredService<IEmbeddingVectorStore>()
                    .UpsertAsync(graph.ChunkEmbedding.Id, [1f, 0f, 0f], TestContext.Current.CancellationToken);
            ...
            IWorkspaceContextSearchStore store =
                readScope.ServiceProvider.GetRequiredService<IWorkspaceContextSearchStore>();
            WorkspaceContextSearchHit hit =
                Assert.Single(await store.SearchAsync(search, TestContext.Current.CancellationToken));
            ...
            Assert.Equal(1d, hit.RecencyScore, 6);
            Assert.Equal(0.5d, hit.ArtifactPriorityScore, 6);
            ...
            IConfiguration vectorOnlyConfiguration = new ConfigurationBuilder()
                ...
                {
                    [$"{WorkspaceContextSearchOptions.SectionName}:{nameof(WorkspaceContextSearchOptions.VectorWeight)}"] =
                        "1",
                    [$"{WorkspaceContextSearchOptions.SectionName}:{nameof(WorkspaceContextSearchOptions.KeywordWeight)}"] =
                        "0",
                    [$"{WorkspaceContextSearchOptions.SectionName}:{nameof(WorkspaceContextSearchOptions.RecencyWeight)}"] =
                        "0",
                    [$"{WorkspaceContextSearchOptions.SectionName}:{nameof(WorkspaceContextSearchOptions.ArtifactPriorityWeight)}"] =
                        "0",
                    [$"{WorkspaceContextSearchOptions.SectionName}:{nameof(WorkspaceContextSearchOptions.SourcePriorityWeight)}"] =
                        "0"
                })
                .Build();
            ...
            IWorkspaceContextSearchStore vectorOnlyStore =
                vectorOnlyScope.ServiceProvider.GetRequiredService<IWorkspaceContextSearchStore>();
            ...
            Assert.Equal(1d, vectorOnlyHit.Score, 6);
        }
