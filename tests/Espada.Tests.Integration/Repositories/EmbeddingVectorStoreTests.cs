...
public sealed class EmbeddingVectorStoreTests(PostgreSqlDatabaseFixture fixture) : PostgreSqlIntegrationTest(fixture)
{
    ...
    public async Task Upsert_ShouldReplaceExistingVector()
    {
        ...
        using (IServiceScope scope = serviceProvider.CreateScope())
        {
            ...
            await scope.ServiceProvider.GetRequiredService<EspadaDbContext>().SaveChangesAsync(TestContext.Current.CancellationToken);
        }
    ...
    public async Task SearchNearest_ShouldReturnMatchingWorkspaceModel()
    {
        ...
        using IServiceScope scope = serviceProvider.CreateScope();
        IEmbeddingVectorStore store = scope.ServiceProvider.GetRequiredService<IEmbeddingVectorStore>();
        IReadOnlyList<EmbeddingVectorSearchHit> hits = await store.SearchNearestAsync(new EmbeddingVectorSearch(graph.Workspace.Id, graph.ChunkEmbedding.Model, [1f, 0f, 0f], 10, 0.99), TestContext.Current.CancellationToken);
    ...
    private static async Task SeedGraphAsync(ServiceProvider serviceProvider, PersistenceGraph graph, IReadOnlyList<float> vector)
    {
        using IServiceScope scope = serviceProvider.CreateScope();
        EspadaDbContext dbContext = scope.ServiceProvider.GetRequiredService<EspadaDbContext>();
        dbContext.AddRange(graph.Workspace, graph.Source, graph.ImportJob, graph.Artifact, graph.ArtifactRevision, graph.ChunkBatch, graph.Chunk, graph.ChunkEmbedding);
        await scope.ServiceProvider.GetRequiredService<IEmbeddingVectorStore>().UpsertAsync(graph.ChunkEmbedding.Id, vector, TestContext.Current.CancellationToken);
        await dbContext.SaveChangesAsync(TestContext.Current.CancellationToken);
    }
}