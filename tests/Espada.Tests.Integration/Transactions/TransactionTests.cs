...
namespace Espada.Tests.Integration.Transactions
{
    ...
    public sealed class TransactionTests(PostgreSqlDatabaseFixture fixture) : PostgreSqlIntegrationTest(fixture)
    {
        ...
        public async Task ArtifactAndFirstRevision_WhenRevisionFails_ShouldPersistNeither()
        {
            ...
            Artifact artifact = Artifact.Create(ArtifactId.Create(Guid.NewGuid()), graph.Workspace.Id,
                ArtifactTitle.Create("Atomic artifact").ShouldSucceed(), ArtifactKindType.Document,
                ArtifactType.Markdown, graph.Artifact.CreatedAtUtc).ShouldSucceed();
            ArtifactRevision revision = artifact.CreateRevision(ArtifactRevisionId.Create(Guid.NewGuid()),
                    ArtifactContent.Create("Atomic revision").ShouldSucceed(), graph.ArtifactRevision.CreatedAtUtc)
                .ShouldSucceed();

            await using EspadaDbContext dbContext = Fixture.CreateDbContext();
            dbContext.AddRange(artifact, revision);
            dbContext.Entry(revision).Property(nameof(ArtifactRevision.ArtifactId)).CurrentValue =
            ...
            Assert.False(await verificationContext.Artifacts.AnyAsync(value => value.Id == artifact.Id,
                TestContext.Current.CancellationToken));
            Assert.False(await verificationContext.ArtifactRevisions.AnyAsync(value => value.Id == revision.Id,
                TestContext.Current.CancellationToken));
        }
        ...
        public async Task ChunkBatchAndChunks_WhenChunkConstraintFails_ShouldPersistNothing()
        {
            ...
            ChunkBatch batch = ChunkBatch.Request(ChunkBatchId.Create(Guid.NewGuid()), graph.Workspace.Id,
                graph.Artifact.Id, graph.ArtifactRevision.Id, ChunkingStrategyType.Recursive,
                ChunkingVersion.Create("recursive-v1").ShouldSucceed(),
                graph.ChunkBatch.RequestedAtUtc).ShouldSucceed();
            ...
            await using EspadaDbContext verificationContext = Fixture.CreateDbContext();
            Assert.False(await verificationContext.ChunkBatches.AnyAsync(value => value.Id == batch.Id,
                TestContext.Current.CancellationToken));
            Assert.False(await verificationContext.Chunks.AnyAsync(value => value.BatchId == batch.Id,
                TestContext.Current.CancellationToken));
        }
        ...
        public async Task EmbeddingMetadataAndVector_WhenMetadataConstraintFails_ShouldPersistNeither()
        {
            ...
            using (IServiceScope setupScope = serviceProvider.CreateScope())
            {
                ...
                setupContext.AddRange(graph.Workspace, graph.Artifact, graph.ArtifactRevision, graph.ChunkBatch,
                    graph.Chunk, graph.ChunkEmbedding);
                await setupScope.ServiceProvider.GetRequiredService<IEmbeddingVectorStore>()
                    .UpsertAsync(graph.ChunkEmbedding.Id, [0.1f, 0.2f, 0.3f], TestContext.Current.CancellationToken);
                await setupContext.SaveChangesAsync(TestContext.Current.CancellationToken);
            }

            ChunkEmbedding duplicate = ChunkEmbedding.Create(ChunkEmbeddingId.Create(Guid.NewGuid()),
                graph.Workspace.Id, graph.Chunk.Id, graph.Chunk.ContentHash, graph.ChunkEmbedding.Model,
                graph.ChunkEmbedding.Dimensions, graph.ChunkEmbedding.CreatedAtUtc).ShouldSucceed();
            ...
            using (IServiceScope failingScope = serviceProvider.CreateScope())
            {
                ...
                await services.GetRequiredService<IChunkEmbeddingRepository>()
                    .AddAsync(duplicate, TestContext.Current.CancellationToken);
                await services.GetRequiredService<IEmbeddingVectorStore>().UpsertAsync(duplicate.Id, [0.4f, 0.5f, 0.6f],
                    TestContext.Current.CancellationToken);
                await Assert.ThrowsAsync<DbUpdateException>(() =>
                    services.GetRequiredService<IUnitOfWork>().SaveChangesAsync(TestContext.Current.CancellationToken));
            }

            using IServiceScope verificationScope = serviceProvider.CreateScope();
            Assert.Null(await verificationScope.ServiceProvider.GetRequiredService<IEmbeddingVectorStore>()
                .GetByIdAsync(duplicate.Id, TestContext.Current.CancellationToken));
        }

        private static Chunk CreateChunk(ChunkBatch batch, int number, string content)
        {
            return Chunk.Create(ChunkId.Create(Guid.NewGuid()), batch.Id, batch.WorkspaceId, batch.ArtifactId,
                batch.ArtifactRevisionId, ChunkNumber.Create(number).ShouldSucceed(),
                ChunkContent.Create(content).ShouldSucceed(), null, batch.Strategy, batch.StrategyVersion,
                batch.RequestedAtUtc).ShouldSucceed();
        }
