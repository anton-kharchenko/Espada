...
public sealed class AutomaticIngestionPipelineTests(PostgreSqlDatabaseFixture fixture) : PostgreSqlIntegrationTest(fixture)
{
    [Fact]
    public async Task PlainTextImport_ShouldRunToSearchableSucceededRevision()
    {
        ...
        try
        {
            ...
            ImportJob completed = await verification.ImportJobs.SingleAsync(value => value.Id == importJobId, TestContext.Current.CancellationToken);
