    ...
    public sealed class BusinessFlowsE2ETests(EspadaE2EFactory factory) : E2ETest(factory)
    {
        ...
        public async Task FullLifecycle_ShouldRoundTripThroughPostgreSql()
        {
            ...
            SourceResponse source =
                await http.GetAsync<SourceResponse>(E2ERoutes.Source(workspace.Id, createdSource.SourceId));
            ...
            GetArtifactByIdResponse artifact =
                await http.GetAsync<GetArtifactByIdResponse>(E2ERoutes.Artifact(workspace.Id,
            ...
            AddArtifactRevisionResponse secondRevision =
                await http.PostAsync<AddArtifactRevisionRequest, AddArtifactRevisionResponse>(
                    ...
                    new AddArtifactRevisionRequest { Content = BusinessFlowTestData.Lifecycle.SecondRevisionContent });

            ListArtifactRevisionsResponse revisions =
                await http.GetAsync<ListArtifactRevisionsResponse>(E2ERoutes.Revisions(workspace.Id, artifact.Id));
            Assert.Equal(2, revisions.Items.Count);
            Assert.Contains(revisions.Items, item => item.Id == secondRevision.ArtifactRevisionId);

            await http.AssertStatusAsync(
                await client.PostAsJsonAsync(E2ERoutes.RenameArtifact(workspace.Id, artifact.Id),
                    new RenameArtifactRequest { Title = BusinessFlowTestData.Lifecycle.RenamedArtifactTitle },
                    TestContext.Current.CancellationToken), HttpStatusCode.NoContent);
            ...
            GetChunkByIdResponse chunk =
                await http.GetAsync<GetChunkByIdResponse>(E2ERoutes.Chunk(workspace.Id, chunkId));
        ...
        public async Task InvalidResourcesAndTransitions_ShouldReturnContractStatusCodes()
        {
            ...
            CreateWorkspaceResponse firstWorkspace =
                await CreateWorkspaceAsync(http, BusinessFlowTestData.InvalidTransitions.FirstWorkspaceName);
            CreateWorkspaceResponse secondWorkspace =
                await CreateWorkspaceAsync(http, BusinessFlowTestData.InvalidTransitions.SecondWorkspaceName);
            await http.AssertStatusAsync(
                await client.GetAsync(
        ...
        {
            return http.PostAsync<CreateWorkspaceRequest, CreateWorkspaceResponse>(E2ERouteConstants.Workspaces,
                new CreateWorkspaceRequest { Name = name, TypeId = WorkspaceType.Personal.Id });
        }

        private static async Task<RequestImportResponse> RequestImportAsync(HttpClient client, Guid workspaceId,
            Guid sourceId)
        {
