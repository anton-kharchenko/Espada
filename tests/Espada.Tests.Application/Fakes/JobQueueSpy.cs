...
internal sealed class JobQueueSpy : IJobQueue
{
    ...
    public Task<IngestionJob?> GetLatestAsync(ImportJobId importJobId, CancellationToken cancellationToken = default) =>
        Task.FromResult<IngestionJob?>(null);

    public Task EnqueueAsync(ImportJobId importJobId, ImportPipelineStageType stage, string idempotencyKey, CancellationToken cancellationToken = default) =>
        throw new NotSupportedException();

    public Task<IngestionJob?> ClaimAsync(string leaseOwner, TimeSpan leaseDuration, CancellationToken cancellationToken = default) =>
        throw new NotSupportedException();
