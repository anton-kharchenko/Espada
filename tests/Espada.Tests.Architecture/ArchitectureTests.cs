...
namespace Espada.Tests.Architecture
{
    public sealed class ArchitectureTests
    {
        ...
        private static readonly Assembly ApiContractsAssembly = typeof(ErrorResponse).Assembly;
        ...
        public void ProductionProjectReferences_ShouldMatchAllowedDependencyGraph()
        {
            IReadOnlyDictionary<string, string[]> expectedReferences = new Dictionary<string, string[]>
            {
                ...
                ["Espada.Worker"] =
                [
                    "Espada.Application", "Espada.Billing", "Espada.Domain", "Espada.Infrastructure",
                    "Espada.ServiceDefaults"
                ],
                ["Aspire"] = ["Espada.Api", "Espada.Daemon", "Espada.Db", "Espada.Mcp", "Espada.Worker"]
            };
            ...
            {
                string projectPath = Directory.GetFiles(Path.Join(repositoryRoot, "src", projectName), "*.csproj",
        ...
        public void Domain_ShouldNotReferenceOuterOrFrameworkLayers()
        {
            ...
            [
                "Espada.", "Microsoft.AspNetCore", "Microsoft.EntityFrameworkCore", "MediatR", "Npgsql",
                "ModelContextProtocol"
            ];
            string[] references =
                DomainAssembly.GetReferencedAssemblies().Select(reference => reference.Name!).ToArray();
        ...
        public void PublicApiContracts_ShouldStayInsideContractsNamespace()
        {
            ...
            Assert.All(publicContracts,
                contract => Assert.StartsWith("Espada.Api.Contracts.", contract.Namespace, StringComparison.Ordinal));
        }
