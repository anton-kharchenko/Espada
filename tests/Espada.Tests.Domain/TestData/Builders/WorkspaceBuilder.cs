...
internal sealed class WorkspaceBuilder
{
    ...
    public WorkspaceBuilder WithId(WorkspaceId id) { _id = id; return this; }

    public WorkspaceBuilder WithName(string name) { _name = CreateName(name); return this; }

    public WorkspaceBuilder WithName(WorkspaceName name) { _name = name; return this; }

    public WorkspaceBuilder WithType(WorkspaceType type) { _type = type; return this; }

    public WorkspaceBuilder CreatedAt(DateTimeOffset createdAtUtc) { _createdAtUtc = createdAtUtc; return this; }
    ...
    public Workspace BuildArchivedWithoutPendingEvents(DateTimeOffset? archivedAtUtc = null)
    {
        ...
        DateTimeOffset effectiveArchivedAtUtc = archivedAtUtc ?? new DateTimeOffset(2026, 7, 26, 18, 0, 0, TimeSpan.Zero);

        DomainResult archiveResult = workspace.Archive(effectiveArchivedAtUtc);

        if (archiveResult.IsFailure)
        {
            throw new InvalidOperationException($"WorkspaceBuilder could not archive workspace: {archiveResult.Error.Code} — {archiveResult.Error.Description}");
        }
