    ...
    public sealed class FileSystemBlobStoreServiceTests
    {
        ...
        public async Task PutSameContentTwice_ShouldReuseSha256Blob()
        {
            ...
            try
            {
                ...
                BlobDescriptor first = await storeService.PutAsync(firstContent, new BlobWriteOptions("text/plain"),
                    TestContext.Current.CancellationToken);
                await using MemoryStream secondContent = new(payload);
                BlobDescriptor second = await storeService.PutAsync(secondContent, new BlobWriteOptions("text/plain"),
                    TestContext.Current.CancellationToken);
                ...
                await using Stream content =
                    await storeService.OpenReadAsync(first.Hash, TestContext.Current.CancellationToken);
