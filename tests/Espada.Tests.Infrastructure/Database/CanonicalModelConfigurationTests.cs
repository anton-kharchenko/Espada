...
public sealed class CanonicalModelConfigurationTests
{
    ...
    {
        using EspadaDbContext context = CreateContext();
        IProperty property = Assert.IsAssignableFrom<IProperty>(context.Model.FindEntityType(typeof(Artifact))?.FindProperty(nameof(Artifact.KindType)));
    ...
    public void BindingWorkspaceSelector_ShouldBeRequired()
    {
        ...
        IForeignKey foreignKey = Assert.Single(metadata.GetForeignKeys(), candidate => candidate.PrincipalEntityType.ClrType == typeof(Workspace));
    ...
    {
        using EspadaDbContext context = CreateContext();
        string[] instructionKey = context.Model.FindEntityType(typeof(InstructionRule))!.FindPrimaryKey()!.Properties.Select(property => property.Name).ToArray();
        string[] policyKey = context.Model.FindEntityType(typeof(PolicyRule))!.FindPrimaryKey()!.Properties.Select(property => property.Name).ToArray();
    ...
    {
        using EspadaDbContext context = CreateContext();
        IEntityType metadata = Assert.IsAssignableFrom<IEntityType>(context.Model.FindEntityType(typeof(ArtifactRevision)));
        IForeignKey foreignKey = Assert.Single(metadata.GetForeignKeys(), candidate => candidate.PrincipalEntityType.ClrType == typeof(Artifact));

        Assert.Equal([nameof(ArtifactRevision.ArtifactId), nameof(ArtifactRevision.WorkspaceId)], foreignKey.Properties.Select(property => property.Name));
        Assert.Equal([nameof(Artifact.Id), nameof(Artifact.WorkspaceId)], foreignKey.PrincipalKey.Properties.Select(property => property.Name));
    }
    ...
    public void Binding_ShouldUseWorkspaceScopedRevisionProjectAndTaskForeignKeys()
    {
        ...
        AssertForeignKey(metadata, typeof(ArtifactRevision), nameof(Binding.ArtifactRevisionId), nameof(Binding.WorkspaceId));
        AssertForeignKey(metadata, typeof(Project), nameof(Binding.ProjectId), nameof(Binding.WorkspaceId));
        AssertForeignKey(metadata, typeof(ProjectTask), nameof(Binding.TaskId), nameof(Binding.ProjectId), nameof(Binding.WorkspaceId));
    }
    ...
    public void RulesAndMemory_ShouldUseKindAndIdentityScopedRevisionForeignKeys()
    {
        ...
        AssertForeignKey(Assert.IsAssignableFrom<IEntityType>(context.Model.FindEntityType(typeof(InstructionRule))), typeof(ArtifactRevision), nameof(InstructionRule.ArtifactRevisionId), nameof(InstructionRule.KindType));
        AssertForeignKey(Assert.IsAssignableFrom<IEntityType>(context.Model.FindEntityType(typeof(PolicyRule))), typeof(ArtifactRevision), nameof(PolicyRule.ArtifactRevisionId), nameof(PolicyRule.KindType));
        AssertForeignKey(Assert.IsAssignableFrom<IEntityType>(context.Model.FindEntityType(typeof(MemoryMetadata))), typeof(ArtifactRevision), nameof(MemoryMetadata.ArtifactRevisionId), nameof(MemoryMetadata.ArtifactId), nameof(MemoryMetadata.KindType));
    }

    private static void AssertForeignKey(IEntityType dependent, Type principalType, params string[] properties)
    {
        IForeignKey foreignKey = Assert.Single(dependent.GetForeignKeys(), candidate => candidate.PrincipalEntityType.ClrType == principalType && candidate.Properties.Select(property => property.Name).SequenceEqual(properties));
        Assert.Equal(DeleteBehavior.Restrict, foreignKey.DeleteBehavior);
    }

    private static EspadaDbContext CreateContext() => new(PostgreSqlDbContextOptions.Create<EspadaDbContext>(ModelTestDatabase.ConnectionString));
}
