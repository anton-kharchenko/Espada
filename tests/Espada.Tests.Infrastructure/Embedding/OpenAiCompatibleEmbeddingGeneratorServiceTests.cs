...
public sealed class OpenAiCompatibleEmbeddingGeneratorServiceTests
{
    ...
    public async Task GenerateAsync_ShouldCallConfiguredModelAndReturnVector()
    {
        ...
        OpenAiCompatibleEmbeddingGeneratorService generatorService = new(client, Options.Create(EmbeddingTestData.CreateOptions(3)));
    ...
    {
        using HttpClient client = new(new DelegateHttpMessageHandler(_ => Task.FromResult(HttpResponseFactory.Json("""{"data":null}"""))));
        OpenAiCompatibleEmbeddingGeneratorService generatorService = new(client, Options.Create(EmbeddingTestData.CreateOptions(3)));
