...
namespace Espada.Infrastructure.Ingestion.Chunking.Strategy
{
    internal class BoundaryChunkingStrategy(string name, IReadOnlyList<string> separators) : IChunkingStrategy
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            while (start < content.Length)
            {
                ...
                if (hardEnd < content.Length)
                {
                    ...
                    {
                        int candidate = content.LastIndexOf(boundary, hardEnd - 1, hardEnd - start,
                ...
                {
                    chunks.Add(new ChunkSegment(number++, content.Substring(segmentStart, segmentEnd - segmentStart),
        ...
        protected virtual IReadOnlyList<string> ResolveSeparators(ImportOptions options)
        {
            return separators;
        }
    }
