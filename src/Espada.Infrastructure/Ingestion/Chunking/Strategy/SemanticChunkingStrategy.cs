...
{
    internal sealed class SemanticChunkingStrategy(IBatchEmbeddingGeneratorService embeddingGenerator)
        : IChunkingStrategy
    {
        public string Name => ChunkingStrategyType.Semantic.Name;

        public async Task<IReadOnlyList<ChunkSegment>> ChunkAsync(string content, ImportOptions options,
            CancellationToken cancellationToken = default)
        {
            ...
            IReadOnlyList<GeneratedEmbedding> embeddings = await embeddingGenerator.GenerateBatchAsync(identifier,
                version, sentences.Select(sentence => sentence.Content).ToArray(), cancellationToken);
            ...
            for (int index = 1; index <= sentences.Count; index++)
            {
                ...
                bool exceedsLength =
                    !isEnd && sentences[index].End - sentences[groupStart].Start > options.MaxCharacters;
                ...
                chunks.Add(new ChunkSegment(chunks.Count + 1, content[start..end], start, end - start));
                groupStart = index;
            }
        ...
        private static (string Identifier, string Version) ParseModel(string? model)
        {
            ...
            return parts.Length == 2 && parts.All(part => part.Length > 0)
                ? (parts[0], parts[1])
                : throw new ArgumentException("Embedding model must use 'identifier@version' format.", nameof(model));
        }
