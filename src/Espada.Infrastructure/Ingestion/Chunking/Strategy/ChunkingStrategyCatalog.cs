...
namespace Espada.Infrastructure.Ingestion.Chunking.Strategy
{
    internal sealed class ChunkingStrategyCatalog
    {
        ...
        public IChunkingStrategy Get(string name)
        {
            return _strategies.TryGetValue(name, out IChunkingStrategy? strategy)
                ? strategy
                : throw new InvalidOperationException($"Chunking strategy '{name}' is not registered.");
        }
    }
}