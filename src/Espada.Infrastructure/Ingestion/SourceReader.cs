...
internal sealed class SourceReader(IBlobStoreService blobStoreService, IConnectorSourceClient connectorSourceClient, IOptions<IngestionOptions> options) : ISourceReader
{
    private const int MaximumRedirects = 10;
    private readonly IngestionOptions _options = options.Value;

    public async Task<SourceReadResult> ReadAsync(SourceDefinition definition, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            result = definition switch
            {
                ...
                PlainTextSourceDefinition text => FromUtf8(text.Content, text.Title, IngestionMediaTypes.PlainText),
                ConversationSourceDefinition conversation => FromUtf8(FormatConversation(conversation.Messages), conversation.Title, IngestionMediaTypes.PlainText),
                ConnectorSourceDefinition connector => FromUtf8(await connectorSourceClient.ReadAsync(connector, timeout.Token), $"{connector.PluginId}-{connector.Resource}.txt", IngestionMediaTypes.PlainText),
                LegacySourceDefinition => throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.LegacySourceUnsupported, "Legacy source definitions must be registered again."),
                _ => throw new IngestionException(JobFailureCategoryType.Poison, IngestionFailureCodes.UnknownSourceDefinition, "Unknown source definition.")
            };
        }
        ...
        if (result.Content.CanSeek && result.Content.Length > _options.MaximumRawBytes)
        {
            ...
            throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.RawSizeLimitExceeded, $"Source exceeds the {_options.MaximumRawBytes}-byte raw limit.");
        }

        return result;
    }

    private async Task<SourceReadResult> ReadFileAsync(FileSourceDefinition definition, CancellationToken cancellationToken)
    {
        if (definition.Blob is not null)
        {
            Stream blobStream = await blobStoreService.OpenReadAsync(new BlobHash(definition.Blob.BlobHash), cancellationToken);
        ...
        if (!file.Exists)
        {
            throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.FileNotFound, "Source file does not exist.");
        }
    ...
    private string ResolveAllowedPath(string requestedPath)
    {
        ...
        foreach (string configuredRoot in _options.AllowedFileRoots)
        {
            ...
            foreach (string part in relative.Split(Path.DirectorySeparatorChar, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (File.Exists(current) || Directory.Exists(current))
                {
                    ...
                    {
                        throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.FileReparsePointRejected, "Source paths may not traverse reparse points.");
                    }
                }
            }
        ...
        throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.FilePathNotAllowed, "Source path is outside the configured roots.");
    }

    private async Task<SourceReadResult> ReadWebPageAsync(WebPageSourceDefinition definition, CancellationToken cancellationToken)
    {
        ...
        for (int redirect = 0; redirect <= MaximumRedirects; redirect++)
        {
            ...
            {
                throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.WebAddressNotPublic, "Web sources must resolve only to public addresses.");
            }
            ...
            if (IsRedirect(response.StatusCode))
            {
                ...
                if (location is null)
                {
                    throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.InvalidRedirect, "Web source returned a redirect without a location.");
                }

                current = location.IsAbsoluteUri ? location : new Uri(current, location);
                if (!string.Equals(current.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase))
                {
                    throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.InsecureRedirect, "Web source redirects must remain HTTPS.");
                }
            ...
            {
                bool transient = response.StatusCode is HttpStatusCode.RequestTimeout or HttpStatusCode.TooManyRequests || (int)response.StatusCode >= 500;
                throw new IngestionException(transient ? JobFailureCategoryType.Transient : JobFailureCategoryType.Permanent, transient ? IngestionFailureCodes.WebSourceUnavailable : IngestionFailureCodes.WebSourceRejected, $"Web source returned HTTP {(int)response.StatusCode}.");
            }
            ...
            content.Position = 0;
            string mediaType = response.Content.Headers.ContentType?.MediaType ?? IngestionMediaTypes.Html;
        ...
        throw new IngestionException(
            ...
            IngestionFailureCodes.RedirectLimitExceeded,
    ...
    private static async Task CopyLimitedAsync(Stream source, Stream destination, long limit, CancellationToken cancellationToken)
    {
        ...
        while ((read = await source.ReadAsync(buffer, cancellationToken)) > 0)
        {
            ...
            if (total > limit)
            {
                throw new IngestionException(JobFailureCategoryType.Permanent, IngestionFailureCodes.RawSizeLimitExceeded, $"Source exceeds the {limit}-byte raw limit.");
            }
