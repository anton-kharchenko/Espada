...
namespace Espada.Infrastructure.Database
{
    internal sealed class PostgreSqlJobQueue(EspadaDbContext dbContext, IClockService clock) : IJobQueue
    {
        ...
        public async Task EnqueueAsync(ImportJobId importJobId, ImportPipelineStageType stage, string idempotencyKey,
        ...
        public async Task<IngestionJob?> ClaimAsync(string leaseOwner, TimeSpan leaseDuration,
            CancellationToken cancellationToken = default)
        {
        ...
        public async Task<IngestionJob?> GetLatestAsync(ImportJobId importJobId,
            CancellationToken cancellationToken = default)
        {
            ArgumentNullException.ThrowIfNull(importJobId);
        ...
        {
            return UpdateTerminalAsync(jobId, leaseOwner, IngestionJobStatusType.Succeeded, null, null,
                cancellationToken);
        }

        public async Task RetryAsync(Guid jobId, string leaseOwner, JobFailureCategoryType category,
            string sanitizedError, DateTimeOffset availableAtUtc, CancellationToken cancellationToken = default)
        {
        ...
        private IQueryable<IngestionJobs> EligibleJobs(DateTimeOffset now, DateTimeOffset availableBefore)
        {
            return dbContext.IngestionJobs
                .AsNoTracking()
                .Where(job =>
                    job.AvailableAtUtc <= availableBefore && (job.Status == (int)IngestionJobStatusType.Pending ||
                                                              (job.Status == (int)IngestionJobStatusType.Running &&
                                                               job.LeaseExpiresAtUtc < now)));
        }

        private async Task UpdateTerminalAsync(Guid jobId, string leaseOwner, IngestionJobStatusType status,
            JobFailureCategoryType? failureCategory, string? sanitizedError, CancellationToken cancellationToken)
        {
            DateTimeOffset completedAtUtc = clock.UtcNow;
        ...
        private static ImportPipelineStageType ReadStage(int id)
        {
            return Enumeration.FromId<ImportPipelineStageType>(id);
        }
    }
}