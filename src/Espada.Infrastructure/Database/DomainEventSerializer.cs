...
internal static class DomainEventSerializer
{
    ...
    public static (string Name, int Version, string Payload) Serialize(IDomainEvent domainEvent) =>
        domainEvent switch
        {
            ...
            (
                ...
                JsonSerializer.Serialize(new ImportRequestedPayload(requested.ImportJobId.Value, requested.SourceId.Value, requested.WorkspaceId.Value, requested.RequestedAtUtc), SerializerOptions)
            ),
            ImportStageScheduledDomainEvent scheduled =>
            (
                DomainEventContractConstants.ImportStageScheduled,
                DomainEventContractConstants.CurrentVersion,
                JsonSerializer.Serialize(new ImportStageScheduledPayload(scheduled.ImportJobId.Value, scheduled.Stage.Id, scheduled.ScheduledAtUtc), SerializerOptions)
            ),
            _ =>
            (
    ...
    private static ImportStageScheduledDomainEvent? DeserializeImportStageScheduled(string payload)
    {
        ...
        return new ImportStageScheduledDomainEvent(ImportJobId.Create(value.ImportJobId), Enumeration.FromId<ImportPipelineStageType>(value.StageId), value.ScheduledAtUtc);
    }

    private static string ToKebabCase(string value) =>
        string.Concat(value.Select((character, index) => index > 0 && char.IsUpper(character) ? $"-{char.ToLowerInvariant(character)}" : char.ToLowerInvariant(character).ToString()));
}