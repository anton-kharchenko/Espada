...
{
    internal sealed class SourceConfiguration : IEntityTypeConfiguration<Source>, IEntityTypeConfiguration<Sources>
    {
        public void Configure(EntityTypeBuilder<Source> builder)
        {
            ...
            ValueComparer<SourceDefinition?> definitionComparer = new(
                (left, right) => left == right || (left != null && right != null
                                                                && SourceDefinitionSerializer.Serialize(left) ==
                                                                SourceDefinitionSerializer.Serialize(right)),
                definition =>
                    definition == null
                        ? 0
                        : SourceDefinitionSerializer.Serialize(definition).GetHashCode(StringComparison.Ordinal),
                definition =>
                    definition == null
                        ? null
                        : SourceDefinitionSerializer.Deserialize(SourceDefinitionSerializer.Serialize(definition)));
            ...
            builder.Property(e => e.Type)
                ...
                .HasConversion(type => type.Id,
                    value => Enumeration.GetAll<SourceType>().Single(type => type.Id == value))
                .IsRequired()
                .UsePropertyAccessMode(PropertyAccessMode.Field);
            ...
            builder.Property(e => e.Status)
                ...
                .HasConversion(status => status.Id,
                    value => Enumeration.GetAll<SourceStatusType>().Single(status => status.Id == value))
                .IsRequired()
                .UsePropertyAccessMode(PropertyAccessMode.Field);
