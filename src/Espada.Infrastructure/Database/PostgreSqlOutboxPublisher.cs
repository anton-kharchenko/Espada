...
internal sealed class PostgreSqlOutboxPublisher(EspadaDbContext dbContext, IDomainEventDispatcherService dispatcherService, IClockService clock) : IOutboxPublisher
{
    ...
    public async Task<bool> PublishNextAsync(string leaseOwner, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            IDomainEvent? domainEvent = DomainEventSerializer.Deserialize(envelope.EventName, envelope.EventVersion, envelope.PayloadJson);
            if (domainEvent is null)
            {
                await MarkProcessedAsync(envelope.EventId, leaseOwner, $"Unsupported outbox contract '{envelope.EventName}' v{envelope.EventVersion}.", cancellationToken);
                return true;
            }
    ...
    private IQueryable<OutboxMessageRecord> EligibleMessages(DateTimeOffset now, DateTimeOffset availableBefore) =>
        dbContext.OutboxMessages
            .AsNoTracking()
            .Where(message => message.ProcessedAtUtc == null && message.AvailableAtUtc <= availableBefore && (message.LeaseExpiresAtUtc == null || message.LeaseExpiresAtUtc < now));

    private async Task MarkProcessedAsync(Guid eventId, string leaseOwner, string? error, CancellationToken cancellationToken)
    {
    ...
    private async Task ReleaseForRetryAsync(Guid eventId, string leaseOwner, string error, CancellationToken cancellationToken)
    {
