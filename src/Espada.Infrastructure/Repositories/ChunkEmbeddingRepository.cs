...
namespace Espada.Infrastructure.Repositories
{
    internal sealed class ChunkEmbeddingRepository(EspadaDbContext dbContext) : IChunkEmbeddingRepository
    {
        ...
        public async Task<ChunkEmbedding?> GetByChunkIdAsync(ChunkId chunkId, EmbeddingModel model,
            ...
            return await dbContext.ChunkEmbeddings
                ...
                .SingleOrDefaultAsync(
                    embedding =>
                        ...
                        model.Identifier &&
                        EF.Property<string>(embedding, DbPropertyConstants.ChunkEmbeddingModelVersion) == model.Version,
                    cancellationToken);
        }

        public async Task<IReadOnlyList<int>> ListDimensionsAsync(WorkspaceId workspaceId, EmbeddingModel model,
            CancellationToken cancellationToken = default)
        {
