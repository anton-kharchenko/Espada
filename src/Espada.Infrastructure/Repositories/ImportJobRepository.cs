...
internal sealed class ImportJobRepository(EspadaDbContext dbContext) : IImportJobRepository
{
    ...
    public Task<ImportJob?> GetByIdempotencyKeyAsync(WorkspaceId workspaceId, string idempotencyKey, CancellationToken cancellationToken = default)
    {
    ...
    public Task<bool> IsBlobReferencedByOtherImportAsync(ImportJobId importJobId, string blobHash, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(importJobId);
