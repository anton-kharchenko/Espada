...
namespace Espada.Infrastructure.Services
{
    ...
    {
        public AzureBlobStoreService(Uri containerUri) : this(new BlobContainerClient(containerUri,
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                await using (FileStream temporary = new(temporaryPath, FileMode.CreateNew, FileAccess.Write,
                ...
                await using FileStream upload = new(temporaryPath, FileMode.Open, FileAccess.Read, FileShare.Read,
        ...
        {
            BlobDownloadStreamingResult response = await container.GetBlobClient(hash.Value)
        ...
        {
            _ = await container.GetBlobClient(hash.Value).DeleteIfExistsAsync(DeleteSnapshotsOption.IncludeSnapshots,
                cancellationToken: cancellationToken);
        }

        public async Task<bool> ExistsAsync(BlobHash hash, CancellationToken cancellationToken)
        {
            return (await container.GetBlobClient(hash.Value).ExistsAsync(cancellationToken)).Value;
        }
    }
}