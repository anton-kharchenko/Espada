...
{
    internal sealed class PostgreSqlBillingStoreService(EspadaDbContext dbContext, IClockService clock)
        : IBillingStoreService
    {
        ...
        public async Task<bool> AddPaymentEventAsync(PaymentEventEnvelope paymentEvent,
            CancellationToken cancellationToken = default)
        {
        ...
        public async Task<ClaimedPaymentEvent?> ClaimPaymentEventAsync(string workerId, TimeSpan leaseDuration,
            CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                return await dbContext.PaymentEvents
                    .AsNoTracking()
                    .Where(paymentEvent => paymentEvent.ProviderEventId == candidateId)
                    .Select(paymentEvent => new ClaimedPaymentEvent(paymentEvent.ProviderEventId,
                        paymentEvent.EventType, paymentEvent.PayloadJson, paymentEvent.Attempt))
                    .SingleAsync(cancellationToken);
            }
        }

        public async Task ApplyCustomerUpdateAsync(BillingCustomerUpdate update,
            CancellationToken cancellationToken = default)
        {
        ...
        public async Task MarkPaymentEventProcessedAsync(string providerEventId, string workerId,
            CancellationToken cancellationToken = default)
        {
        ...
        public async Task MarkPaymentEventFailedAsync(string providerEventId, string workerId, bool retryable,
            DateTimeOffset availableAtUtc, string sanitizedError, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<ClaimedUsageReconciliation?> ClaimUsageReconciliationAsync(string workerId,
            TimeSpan leaseDuration, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                await MarkUsageReconciliationFailedAsync(eventId, workerId, false, now,
                    "Usage reconciliation references missing billing data.", cancellationToken);
            }
        }

        public async Task MarkUsageReconciledAsync(Guid eventId, string workerId,
            CancellationToken cancellationToken = default)
        {
            DateTimeOffset processedAtUtc = clock.UtcNow;
        ...
        public async Task MarkUsageReconciliationFailedAsync(Guid eventId, string workerId, bool retryable,
            DateTimeOffset availableAtUtc, string sanitizedError, CancellationToken cancellationToken = default)
        {
            UsageReconciliationStatusType status =
                retryable ? UsageReconciliationStatusType.Pending : UsageReconciliationStatusType.Failed;
        ...
        {
            return dbContext.PaymentEvents
                .AsNoTracking()
                .Where(paymentEvent => paymentEvent.AvailableAtUtc <= availableBefore &&
                                       (paymentEvent.Status == (int)PaymentEventStatusType.Pending ||
                                        (paymentEvent.Status == (int)PaymentEventStatusType.Processing &&
                                         paymentEvent.LeaseExpiresAtUtc <= now)));
        }
        ...
        {
            return dbContext.UsageReconciliationOutbox
                .AsNoTracking()
                .Where(message => message.AvailableAtUtc <= availableBefore &&
                                  (message.Status == (int)UsageReconciliationStatusType.Pending ||
                                   (message.Status == (int)UsageReconciliationStatusType.Processing &&
                                    message.LeaseExpiresAtUtc <= now)))
                .Where(message => dbContext.UsageLedgerEntries.Any(ledger =>
                    ledger.EntryId == message.LedgerEntryId &&
                    dbContext.BillingCustomers.Any(customer => customer.WorkspaceId == ledger.WorkspaceId)));
        }

        private IQueryable<PaymentEvents> ProcessingPaymentEvent(string providerEventId, string workerId)
        {
            return dbContext.PaymentEvents.Where(paymentEvent =>
                paymentEvent.ProviderEventId == providerEventId &&
                paymentEvent.Status == (int)PaymentEventStatusType.Processing && paymentEvent.LeaseOwner == workerId);
        }

        private IQueryable<UsageReconciliationOutbox> ProcessingUsageReconciliation(Guid eventId, string workerId)
        {
            return dbContext.UsageReconciliationOutbox.Where(message =>
                message.EventId == eventId && message.Status == (int)UsageReconciliationStatusType.Processing &&
                message.LeaseOwner == workerId);
        }

        private async Task UpsertCustomerAsync(Guid workspaceId, CloudBillingPlanType plan,
            BillingCustomerUpdate update, CancellationToken cancellationToken)
        {
            int updated = await UpdateCustomerByWorkspaceAsync(workspaceId, plan, update, cancellationToken);
            if (updated > 0 ||
                await dbContext.BillingCustomers.AnyAsync(customer => customer.WorkspaceId == workspaceId,
                    cancellationToken))
            {
                return;
            }
        ...
        private Task<int> UpdateCustomerByWorkspaceAsync(Guid workspaceId, CloudBillingPlanType plan,
            BillingCustomerUpdate update, CancellationToken cancellationToken)
        {
            return dbContext.BillingCustomers
                .Where(customer => customer.WorkspaceId == workspaceId &&
                                   customer.LastProviderEventAtUtc <= update.ProviderEventAtUtc)
                .ExecuteUpdateAsync(
                    setters => setters
                        .SetProperty(customer => customer.ProviderCustomerId, update.ProviderCustomerId)
                        .SetProperty(customer => customer.ProviderSubscriptionId,
                            customer => update.ProviderSubscriptionId ?? customer.ProviderSubscriptionId)
        ...
        private Task<int> UpdateCustomerByProviderIdAsync(BillingCustomerUpdate update,
            CancellationToken cancellationToken)
        {
            return dbContext.BillingCustomers
                .Where(customer => customer.ProviderCustomerId == update.ProviderCustomerId &&
                                   customer.LastProviderEventAtUtc <= update.ProviderEventAtUtc)
                .ExecuteUpdateAsync(
                    setters => setters
                        .SetProperty(customer => customer.ProviderSubscriptionId,
                            customer => update.ProviderSubscriptionId ?? customer.ProviderSubscriptionId)
                        .SetProperty(customer => customer.Plan,
                            customer => update.Plan.HasValue ? (int)update.Plan.Value : customer.Plan)
                        .SetProperty(customer => customer.SubscriptionStatus, update.SubscriptionStatus)
                        .SetProperty(customer => customer.PaymentFailedAtUtc, update.PaymentFailedAtUtc)
                        .SetProperty(customer => customer.LastProviderEventAtUtc, update.ProviderEventAtUtc),
                    cancellationToken);
