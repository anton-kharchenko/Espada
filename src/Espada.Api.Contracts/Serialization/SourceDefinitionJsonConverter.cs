...
public sealed class SourceDefinitionJsonConverter : JsonConverter<SourceDefinition>
{
    public override SourceDefinition Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (!document.RootElement.TryGetProperty(SourceDefinitionDiscriminatorConstants.Property, out JsonElement typeElement))
        {
        ...
        return typeElement.GetString() switch
        {
            ...
            SourceDefinitionDiscriminatorConstants.PlainText => Deserialize<PlainTextSourceDefinition>(json, options),
            SourceDefinitionDiscriminatorConstants.Conversation => Deserialize<ConversationSourceDefinition>(json, options),
            SourceDefinitionDiscriminatorConstants.Connector => Deserialize<ConnectorSourceDefinition>(json, options),
    ...
    private static T Deserialize<T>(string json, JsonSerializerOptions options) where T : SourceDefinition => JsonSerializer.Deserialize<T>(json, options) ?? throw new JsonException("Source definition payload was empty.");
}