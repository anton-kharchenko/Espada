...
namespace Espada.Api.Contracts.Requests.ChunkBatches
{
    ...
    {
        [Range(1, int.MaxValue)] public int StrategyId { get; init; }
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (!supported)
            {
                yield return new ValidationResult($"Unsupported chunking strategy ID '{StrategyId}'.",
                    [nameof(StrategyId)]);
            }

            if (string.IsNullOrWhiteSpace(StrategyVersion))
            {
                yield return new ValidationResult("Chunking strategy version cannot be empty.",
