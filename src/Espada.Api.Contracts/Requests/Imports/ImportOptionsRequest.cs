...
{
    private static readonly HashSet<string> SupportedStrategies = new(["FixedSize", "Recursive", "Markdown", "Semantic", "Code", "Custom"], StringComparer.OrdinalIgnoreCase);
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            yield return new ValidationResult($"Unsupported chunking strategy '{ChunkingStrategy}'.", [nameof(ChunkingStrategy)]);
        }
        ...
        {
            yield return new ValidationResult("OverlapCharacters must be non-negative and less than MaxCharacters.", [nameof(OverlapCharacters)]);
        }

        if (SemanticThreshold is < 0 or > 1)
        {
            yield return new ValidationResult("SemanticThreshold must be between 0 and 1.", [nameof(SemanticThreshold)]);
        }
    }
}