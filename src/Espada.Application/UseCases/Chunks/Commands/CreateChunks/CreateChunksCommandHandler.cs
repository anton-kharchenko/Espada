...
namespace Espada.Application.UseCases.Chunks.Commands.CreateChunks
{
    ...
    {
        public async Task<DomainResult<CreateChunksResponse>> Handle(CreateChunksCommand request,
            CancellationToken cancellationToken)
        {
            ...
            if (batch is null)
            {
                return DomainResult<CreateChunksResponse>.Failure(
                    ChunkBatchApplicationErrors.NotFound(request.ChunkBatchId));
            }

            if (batch.WorkspaceId.Value != request.WorkspaceId)
            {
                return DomainResult<CreateChunksResponse>.Failure(
                    ChunkBatchApplicationErrors.NotFoundInWorkspace(request.ChunkBatchId, request.WorkspaceId));
            }
            ...
            foreach (CreateChunkItem item in request.Items.OrderBy(value => value.Number))
            {
                ...
                if (item.SourceStart.HasValue && item.SourceLength.HasValue)
                {
                    DomainResult<SourceTextSpan> sourceSpanResult =
                        SourceTextSpan.Create(item.SourceStart.Value, item.SourceLength.Value);
                ...
                DomainResult<Chunk> chunkResult = Chunk.Create(ChunkId.New(), batch.Id, batch.WorkspaceId,
                    batch.ArtifactId, batch.ArtifactRevisionId, numberResult.Value, contentResult.Value, sourceSpan,
                    batch.Strategy, batch.StrategyVersion, startedAtUtc);
            ...
            CreatedChunkResponse[] items = chunks.Select(chunk => new CreatedChunkResponse(chunk.Id.Value,
                chunk.Number.Value, chunk.ContentHash.Value, chunk.SizeInBytes, chunk.CharacterCount)).ToArray();
            return DomainResult<CreateChunksResponse>.Success(new CreateChunksResponse(batch.Id.Value, chunks.Count,
                completedAtUtc, items));
        }
    }
}