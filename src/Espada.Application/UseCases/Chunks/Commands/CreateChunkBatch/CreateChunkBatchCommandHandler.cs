...
namespace Espada.Application.UseCases.Chunks.Commands.CreateChunkBatch
{
    ...
    {
        public async Task<DomainResult<CreateChunkBatchResponse>> Handle(CreateChunkBatchCommand request,
            CancellationToken cancellationToken)
        {
            ...
            ChunkingStrategyType? strategy = Enumeration.GetAll<ChunkingStrategyType>()
                .SingleOrDefault(value => value.Id == request.StrategyId);

            if (strategy is null)
            {
                return DomainResult<CreateChunkBatchResponse>.Failure(
                    ChunkBatchApplicationErrors.UnsupportedStrategy(request.StrategyId));
            }
            ...
            {
                return DomainResult<CreateChunkBatchResponse>.Failure(
                    ArtifactRevisionApplicationErrors.NotFoundInArtifact(request.ArtifactRevisionId,
                        request.ArtifactId));
            }

            DateTimeOffset requestedAtUtc = clockService.UtcNow;
            DomainResult<ChunkBatch> batchResult = ChunkBatch.Request(ChunkBatchId.New(), artifact.WorkspaceId,
                artifact.Id, revision.Id, strategy, versionResult.Value, requestedAtUtc);
            ...
            CreateChunkBatchResponse response = new(batch.Id.Value, batch.ArtifactRevisionId.Value, batch.Strategy.Id,
                batch.Strategy.Name, batch.StrategyVersion.Value, batch.Status.Id, batch.Status.Name,
