...
{
    public async Task<DomainResult<ListChunksByRevisionResponse>> Handle(ListChunksByRevisionQuery request, CancellationToken cancellationToken)
    {
        ...
        if (revision is null)
        {
            return DomainResult<ListChunksByRevisionResponse>.Failure(ArtifactRevisionApplicationErrors.NotFound(request.ArtifactRevisionId));
        }
        ...
        IReadOnlyList<Chunk> chunks = await chunkRepository.ListByArtifactRevisionIdAsync(revision.Id, cancellationToken);

        ChunkListItemResponse[] items = chunks
            .OrderBy(chunk => chunk.Number.Value)
            .Select(chunk => new ChunkListItemResponse(chunk.Id.Value, chunk.BatchId.Value, chunk.Number.Value, chunk.ContentHash.Value, chunk.SizeInBytes, chunk.CharacterCount, chunk.SourceSpan?.Start, chunk.SourceSpan?.Length, chunk.CreatedAtUtc))
            .ToArray();
