...
namespace Espada.Application.UseCases.ChunkEmbeddings.Commands.GenerateChunkEmbedding
{
    ...
    {
        public async Task<DomainResult<CreateChunkEmbeddingResponse>> Handle(GenerateChunkEmbeddingCommand request,
            CancellationToken cancellationToken)
        {
            ...
            GeneratedEmbedding generated = await embeddingGeneratorService.GenerateAsync(modelResult.Value.Identifier,
                modelResult.Value.Version, chunk.Content.Value, cancellationToken);
            ...
            if (storedDimensions.Count == 1 && storedDimensions[0] != dimensions)
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.DimensionMismatch(modelResult.Value.Identifier,
                        modelResult.Value.Version, storedDimensions[0], dimensions));
            }
            ...
            return DomainResult<CreateChunkEmbeddingResponse>.Success(
                mapper.Map<CreateChunkEmbeddingResponse>(embedding));
        }
    }
}