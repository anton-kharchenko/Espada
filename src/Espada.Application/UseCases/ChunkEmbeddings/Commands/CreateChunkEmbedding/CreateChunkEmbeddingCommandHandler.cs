...
namespace Espada.Application.UseCases.ChunkEmbeddings.Commands.CreateChunkEmbedding
{
    ...
    {
        public async Task<DomainResult<CreateChunkEmbeddingResponse>> Handle(CreateChunkEmbeddingCommand request,
            CancellationToken cancellationToken)
        {
            ...
            DomainResult<EmbeddingModel> modelResult =
                EmbeddingModel.Create(request.ModelIdentifier, request.ModelVersion);
            ...
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(ChunkEmbeddingApplicationErrors
                    .VectorContainsNonFiniteValue);
            }
            ...
            if (chunk is null)
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(
                    ChunkApplicationErrors.NotFound(request.ChunkId));
            }
            ...
            IReadOnlyList<int> storedDimensions =
                await chunkEmbeddingRepository.ListDimensionsAsync(chunk.WorkspaceId, modelResult.Value,
                    cancellationToken);

            if (storedDimensions.Count > 1)
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.InconsistentModelDimensions(modelResult.Value.Identifier,
                        modelResult.Value.Version));
            }

            if (storedDimensions.Count == 1 && storedDimensions[0] != vector.Count)
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.DimensionMismatch(modelResult.Value.Identifier,
                        modelResult.Value.Version, storedDimensions[0], vector.Count));
            }

            ChunkEmbedding? existing =
                await chunkEmbeddingRepository.GetByChunkIdAsync(chunk.Id, modelResult.Value, cancellationToken);
            if (existing is not null)
            {
                return DomainResult<CreateChunkEmbeddingResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.AlreadyExistsForModel(chunk.Id.Value, modelResult.Value.Identifier,
                        modelResult.Value.Version));
            }

            DateTimeOffset createdAtUtc = clockService.UtcNow;
            DomainResult<ChunkEmbedding> embeddingResult = ChunkEmbedding.Create(ChunkEmbeddingId.New(),
                chunk.WorkspaceId, chunk.Id, chunk.ContentHash, modelResult.Value, dimensionsResult.Value,
                createdAtUtc);
            ...
            CreateChunkEmbeddingResponse response = new(embedding.Id.Value, embedding.ChunkId.Value,
                embedding.ChunkContentHash.Value, embedding.Model.Identifier, embedding.Model.Version,
                embedding.Dimensions.Value, embedding.CreatedAtUtc);
