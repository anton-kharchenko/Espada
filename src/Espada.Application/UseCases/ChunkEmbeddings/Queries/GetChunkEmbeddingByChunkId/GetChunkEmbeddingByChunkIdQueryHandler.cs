...
namespace Espada.Application.UseCases.ChunkEmbeddings.Queries.GetChunkEmbeddingByChunkId
{
    internal sealed class GetChunkEmbeddingByChunkIdQueryHandler(
        ...
        : IQueryHandler<GetChunkEmbeddingByChunkIdQuery, GetChunkEmbeddingByChunkIdResponse>
    {
        public async Task<DomainResult<GetChunkEmbeddingByChunkIdResponse>> Handle(
            GetChunkEmbeddingByChunkIdQuery request, CancellationToken cancellationToken)
        {
            ...
            if (chunk is null)
            {
                return DomainResult<GetChunkEmbeddingByChunkIdResponse>.Failure(
            ...
            {
                return DomainResult<GetChunkEmbeddingByChunkIdResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.NotFoundForChunk(request.ChunkId));
            }
            ...
            {
                return DomainResult<GetChunkEmbeddingByChunkIdResponse>.Failure(
                    ChunkEmbeddingApplicationErrors.VectorNotFound(embedding.Id.Value));
            }

            GetChunkEmbeddingByChunkIdResponse response = new(embedding.Id.Value, embedding.ChunkId.Value,
                embedding.ChunkContentHash.Value, embedding.Model.Identifier, embedding.Model.Version,
                embedding.Dimensions.Value, vector, embedding.CreatedAtUtc);
