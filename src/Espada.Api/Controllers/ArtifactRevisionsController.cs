...
namespace Espada.Api.Controllers
{
    ...
    public sealed class ArtifactRevisionsController(IMediator mediator) : BaseController
    {
        ...
        public async Task<IActionResult> Add([FromRoute] Guid workspaceId, [FromRoute] Guid artifactId,
            [FromBody] AddArtifactRevisionRequest request, CancellationToken cancellationToken)
        {
            DomainResult<AddArtifactRevisionResponse> result = await mediator.Send(
                new AddArtifactRevisionCommand(workspaceId, artifactId, request.Content), cancellationToken);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> List([FromRoute] Guid workspaceId, [FromRoute] Guid artifactId,
            CancellationToken cancellationToken)
        {
            DomainResult<ListArtifactRevisionsResponse> result =
                await mediator.Send(new ListArtifactRevisionsQuery(workspaceId, artifactId), cancellationToken);
        ...
        {
            DomainResult<GetArtifactRevisionByIdResponse> result = await mediator.Send(
                new GetArtifactRevisionByIdQuery(workspaceId, artifactId, artifactRevisionId), cancellationToken);
