...
public sealed class BlobsController(IBlobStoreService blobStoreService, IOptions<IngestionOptions> ingestionOptions) : BaseController
{
    [HttpPost]
    ...
    public async Task<IActionResult> Upload([FromRoute] Guid workspaceId, [FromHeader(Name = "X-File-Name"), Required] string fileName, CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(normalizedFileName, fileName, StringComparison.Ordinal) || normalizedFileName is "." or "..")
        {
            return BadRequest("X-File-Name must contain a file name without a path.");
        }

        long maximumBytes = Math.Min(IngestionConstants.DefaultMaximumRawBytes, ingestionOptions.Value.MaximumRawBytes);
        ...
        BlobDescriptor blob = await blobStoreService.PutAsync(Request.Body, new BlobWriteOptions(mediaType), cancellationToken);
