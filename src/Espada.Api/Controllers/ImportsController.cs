...
public sealed class ImportsController(IMediator mediator, IMapper mapper) : BaseController
{
    ...
    public async Task<IActionResult> RequestImport([FromRoute] Guid workspaceId, [FromHeader(Name = HttpHeaderNames.IdempotencyKey), Required] string idempotencyKey, [FromBody] RequestImportRequest request, CancellationToken cancellationToken)
    {
        RequestImportCommand command = mapper.Map<RequestImportCommand>(new RequestImportMappingSource(workspaceId, idempotencyKey, request));
        ...
        return result.IsFailure
            ...
            : AcceptedAtAction(
                ...
                {
                    workspaceId,
                    importJobId = result.Value.ImportJobId,
    ...
    {
        DomainResult result = await mediator.Send(new CancelImportCommand(workspaceId, importJobId), cancellationToken);
    ...
    {
        DomainResult<GetImportByIdResponse> result = await mediator.Send(new GetImportByIdQuery(workspaceId, importJobId), cancellationToken);
