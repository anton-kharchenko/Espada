...
namespace Espada.Api.Controllers
{
    ...
    public sealed class BillingController(IMediator mediator, IMapper mapper) : BaseController
    {
        ...
        public async Task<IActionResult> CreateCheckout([FromRoute] Guid workspaceId,
            [FromBody] CreateCheckoutRequest request,
            [FromHeader(Name = HttpHeaderNameConstants.IdempotencyKey)] string idempotencyKey,
            CancellationToken cancellationToken)
        {
            CreateCheckoutCommand command =
                mapper.Map<CreateCheckoutCommand>(new CreateCheckoutMappingSource(workspaceId, request,
                    idempotencyKey));
        ...
        [ProducesResponseType<HostedBillingSession>(StatusCodes.Status202Accepted)]
        public async Task<IActionResult> CreatePortal([FromRoute] Guid workspaceId,
            [FromHeader(Name = HttpHeaderNameConstants.IdempotencyKey)] string idempotencyKey,
            CancellationToken cancellationToken)
        {
            DomainResult<HostedBillingSession> result =
                await mediator.Send(new CreateCustomerPortalCommand(workspaceId, idempotencyKey), cancellationToken);
        ...
        {
            DomainResult<BillingStatusSnapshot> result =
                await mediator.Send(new GetBillingStatusQuery(workspaceId), cancellationToken);
        ...
        [RequestSizeLimit(BillingRequestLimitConstants.MaximumWebhookPayloadBytes)]
        ...
        public async Task<IActionResult> StripeWebhook(CancellationToken cancellationToken)
        {
            ...
            string signature = Request.Headers[HttpHeaderNameConstants.StripeSignature].ToString();
            DomainResult<StripeWebhookReceipt> result =
                await mediator.Send(new AcceptStripeWebhookCommand(payload, signature), cancellationToken);

            return result.IsFailure
                ? HandleError(result.Error)
                : Ok(mapper.Map<StripeWebhookReceiptResponse>(result.Value));
        }
    }
}