...
internal sealed class ChunkConfiguration : IEntityTypeConfiguration<Models.Chunks>
{
    public void Configure(EntityTypeBuilder<Models.Chunks> builder)
    {
        ...
        builder.HasOne<Models.ChunkBatches>().WithMany().HasForeignKey(model => model.ChunkBatchId).OnDelete(DeleteBehavior.Restrict);
