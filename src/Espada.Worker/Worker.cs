...
namespace Espada.Worker
{
    public sealed class Worker(IServiceScopeFactory scopeFactory, ILogger<Worker> logger) : BackgroundService
    {
        ...
        private async Task<bool> ReconcileUsageAsync(CancellationToken cancellationToken)
        {
            ...
            IUsageReconciliationProcessor?
                processor = scope.ServiceProvider.GetService<IUsageReconciliationProcessor>();
        ...
        {
            using IServiceScope scope = scopeFactory.CreateScope();
            return await scope.ServiceProvider.GetRequiredService<IOutboxPublisher>()
                .PublishNextAsync(_workerId, cancellationToken);
        }
        ...
        private async Task<bool> ProcessJobAsync(CancellationToken stoppingToken)
        {
            ...
            try
            {
                ...
                await executionScope.ServiceProvider.GetRequiredService<IImportPipelineStageExecutorService>()
                    .ExecuteAsync(job, stoppingToken);
                await executionScope.ServiceProvider.GetRequiredService<IJobQueue>()
                    .CompleteAsync(job.JobId, _workerId, stoppingToken);
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            {
                throw;
            ...
            {
                await FinishFailedJobAsync(job, JobFailureCategoryType.Cancelled, "Import was cancelled.",
                    stoppingToken);
            }
            ...
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                ...
                await FinishFailedJobAsync(job, JobFailureCategoryType.Poison,
                    "The job payload or stage handler was invalid.", stoppingToken);
            }

            return true;
        }

        private async Task FinishFailedJobAsync(IngestionJob job, JobFailureCategoryType category,
            string sanitizedError, CancellationToken cancellationToken)
        {
            ...
            await queue.RetryAsync(job.JobId, _workerId, effectiveCategory, Sanitize(sanitizedError), availableAtUtc,
                cancellationToken);

            if (effectiveCategory != JobFailureCategoryType.Transient)
            {
                await MarkImportTerminalAsync(scope.ServiceProvider, job.ImportJobId, effectiveCategory, sanitizedError,
                    cancellationToken);
            }
        }
        ...
        {
            return exception is HttpRequestException or IOException or TimeoutException or NpgsqlException;
        }

        private static string Sanitize(string value)
        {
            string normalized = value.Replace('\r', ' ').Replace('\n', ' ').Trim();
            return normalized.Length <= ImportFailure.ReasonMaxLength
                ? normalized
                : normalized[..ImportFailure.ReasonMaxLength];
        }
    }
}