...
namespace Espada.Billing.Extensions
{
    public static class BillingServiceCollectionExtensions
    {
        public static void AddEspadaBilling(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (!string.Equals(StripeConfiguration.ApiVersion, BillingConstants.RequiredStripeApiVersion,
                    StringComparison.Ordinal))
            {
                throw new InvalidOperationException(
                    $"Stripe.net API version must be {BillingConstants.RequiredStripeApiVersion}, but is {StripeConfiguration.ApiVersion}.");
            }
