...
namespace Espada.Billing.Options
{
    public sealed class BillingUsageOptions
    {
        ...
        public bool IsValid()
        {
            return new[]
            {
                RawBytesEventName, ExtractedBytesEventName, EmbeddingInputUnitsEventName,
                ParserComputeMillisecondsEventName, PluginComputeMillisecondsEventName, EgressBytesEventName,
                StorageByteHoursEventName
            }.All(value => !string.IsNullOrWhiteSpace(value));
        }

        public string GetEventName(string metric)
        {
