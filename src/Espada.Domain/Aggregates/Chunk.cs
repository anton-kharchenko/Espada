...
public sealed class Chunk : AggregateRoot<ChunkId>
{
    ...
    public ChunkBatchId BatchId { get; private set; } = null!;
    ...
    public ChunkNumber Number { get; private set; } = null!;
        ...
        DateTimeOffset createdAtUtc)
    {
        ...
        Chunk chunk = new(id, batchId, workspaceId, artifactId, artifactRevisionId, number, content, sourceSpan, strategy, strategyVersion, createdAtUtc);

        chunk.RaiseDomainEvent(new ChunkCreatedDomainEvent(chunk.Id, chunk.BatchId, chunk.WorkspaceId, chunk.ArtifactId, chunk.ArtifactRevisionId, chunk.Number.Value, chunk.ContentHash.Value, chunk.SizeInBytes, chunk.SourceSpan?.Start, chunk.SourceSpan?.Length, chunk.Strategy, chunk.StrategyVersion.Value, createdAtUtc));
