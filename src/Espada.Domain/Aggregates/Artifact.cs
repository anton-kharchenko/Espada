...
public sealed class Artifact : AggregateRoot<ArtifactId>, IHasConcurrencyVersion
{
        ...
        DateTimeOffset createdAtUtc)
    {
        ...
        return Owns(revision)
            ? Binding.Create(bindingId, revision, workspace, organizationId, project, repositoryCanonicalUri, repositoryRelativePathPrefix, branch, task, agent, createdAtUtc)
            : DomainResult<Binding>.Failure(BindingErrors.RevisionMismatch);
    }

    public DomainResult<InstructionRule> CreateInstructionRule(ArtifactRevision revision, RuleKey ruleKey, string? text, ContextPriority priority)
    {
        ArgumentNullException.ThrowIfNull(revision);
    ...
    public DomainResult<PolicyRule> CreatePolicyRule(ArtifactRevision revision, RuleKey ruleKey, string? text, ContextPriority priority, PolicyEnforcementType enforcementType)
    {
        ArgumentNullException.ThrowIfNull(revision);
        ...
        MemoryId? supersededMemoryId = null)
    {
        ...
        return Owns(revision)
            ? MemoryMetadata.Create(memoryId, revision, categoryType, confidence, userConfirmed, clientIdentity, sessionIdentity, capturedAtUtc, supersededMemoryId)
            : DomainResult<MemoryMetadata>.Failure(MemoryErrors.RevisionMismatch);
    }

    private bool Owns(ArtifactRevision revision) =>
        revision.ArtifactId.Equals(Id)
    ...
    public DomainResult<ArtifactRevision> CreateRevision(ArtifactRevisionId revisionId, ArtifactContent content, DateTimeOffset createdAtUtc)
    {
        ...
        ArtifactRevision revision = ArtifactRevision.Create(revisionId, Id, WorkspaceId, KindType, nextNumber, content, createdAtUtc);
