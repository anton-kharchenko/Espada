...
{
    public override ArtifactKindType Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        string? identifier = reader.GetString();
        return Enumeration.GetAll<ArtifactKindType>().SingleOrDefault(kind => kind.Name == identifier) ?? throw new JsonException($"Unknown artifact kindType '{identifier}'.");
    }
