...
public sealed class SourceTextSpan : ValueObject
{
    ...
    public static DomainResult<SourceTextSpan> Create(int start, int length)
    {
        ...
        return start > int.MaxValue - length ? DomainResult<SourceTextSpan>.Failure(ChunkErrors.SourceSpanOverflow) : DomainResult<SourceTextSpan>.Success(new SourceTextSpan(start, length));
    }
