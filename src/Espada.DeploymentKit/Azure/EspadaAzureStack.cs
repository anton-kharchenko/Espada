...
namespace Espada.DeploymentKit.Azure
{
    internal static class EspadaAzureStack
    {
        public static IDictionary<string, object?> Create(DeploymentSettings settings)
        {
            ...
            Workspace workspace = new(names.LogAnalytics,
                new WorkspaceArgs
                {
            ...
            Component insights = new(names.ApplicationInsights,
                new ComponentArgs
                {
                ...
                new ManagedEnvironmentArgs
                {
                    ...
                    AppLogsConfiguration = new AppLogsConfigurationArgs
                    {
                        ...
                        LogAnalyticsConfiguration = new LogAnalyticsConfigurationArgs
                        {
                            ...
                            SharedKey = Output.CreateSecret(workspaceKeys.Apply(keys =>
                                keys.PrimarySharedKey ?? string.Empty))
                        }
                    },
                    Tags = tags
                });
            ...
            UserAssignedIdentity apiIdentity =
                CreateIdentity(names.ApiIdentity, settings.Location, resourceGroup, tags);
            UserAssignedIdentity mcpIdentity =
                CreateIdentity(names.McpIdentity, settings.Location, resourceGroup, tags);
            UserAssignedIdentity workerIdentity =
                CreateIdentity(names.WorkerIdentity, settings.Location, resourceGroup, tags);
            UserAssignedIdentity migrationIdentity =
                CreateIdentity(names.MigrationIdentity, settings.Location, resourceGroup, tags);
            RoleAssignment apiRegistryPull = CreateRoleAssignment(
                ...
                apiIdentity);
            RoleAssignment mcpRegistryPull = CreateRoleAssignment(
                $"{names.Mcp}-acr-pull",
                settings.SubscriptionId,
                registry.Id,
                mcpIdentity.PrincipalId,
                AzureBuiltInRoleDefinitionIdConstants.AcrPull,
                mcpIdentity);
            ...
            Vault vault = new(names.KeyVault,
                new VaultArgs
                {
            ...
            RoleAssignment apiVaultAccess = CreateRoleAssignment(
                ...
                apiIdentity);
            RoleAssignment mcpVaultAccess = CreateRoleAssignment(
                $"{names.Mcp}-vault-secrets",
                settings.SubscriptionId,
                vault.Id,
                mcpIdentity.PrincipalId,
                AzureBuiltInRoleDefinitionIdConstants.KeyVaultSecretsUser,
            ...
            StorageAccount storage = new(names.StorageAccount,
            ...
            BlobContainer ingestionBlobs = new($"{names.StorageAccount}-ingestion",
            ...
            RoleAssignment apiBlobAccess = CreateRoleAssignment(
                ...
                apiIdentity);
            RoleAssignment mcpBlobAccess = CreateRoleAssignment(
                $"{names.Mcp}-blob-data",
                settings.SubscriptionId,
                storage.Id,
                mcpIdentity.PrincipalId,
            ...
            Database database = new(names.PostgreSqlDatabase,
                new DatabaseArgs
                {
            ...
            Secret administratorConnectionStringSecret = CreateVaultSecret(
                ...
                tags,
                deploymentVaultAdministrator);
            Secret mcpEntraClientSecret = CreateVaultSecret(
                AzureDeploymentConstants.McpEntraClientSecret,
                Output.CreateSecret(settings.McpEntraClientSecret!),
                vault,
                resourceGroup,
                tags,
                deploymentVaultAdministrator);
            Secret mcpSigningCertificate = CreateVaultSecret(
                AzureDeploymentConstants.McpSigningCertificateSecret,
                Output.CreateSecret(settings.McpSigningCertificate!),
                vault,
                resourceGroup,
                tags,
                deploymentVaultAdministrator);
            Secret mcpEncryptionCertificate = CreateVaultSecret(
                AzureDeploymentConstants.McpEncryptionCertificateSecret,
                Output.CreateSecret(settings.McpEncryptionCertificate!),
                vault,
                resourceGroup,
                tags,
                deploymentVaultAdministrator);
            ...
            Output<string> apiImage = registry.LoginServer.Apply(loginServer =>
                $"{loginServer}/{AzureDeploymentConstants.ApiImageRepository}:{settings.ImageTag}");
            Output<string> databaseImage = registry.LoginServer.Apply(loginServer =>
                $"{loginServer}/{AzureDeploymentConstants.DatabaseImageRepository}:{settings.ImageTag}");
            Output<string> mcpImage = registry.LoginServer.Apply(loginServer =>
                $"{loginServer}/{AzureDeploymentConstants.McpImageRepository}:{settings.ImageTag}");
            Output<string> workerImage = registry.LoginServer.Apply(loginServer =>
                $"{loginServer}/{AzureDeploymentConstants.WorkerImageRepository}:{settings.ImageTag}");
            ...
            Output<string> mcpIssuer = Output.Format(
                $"https://{names.Mcp}.{containerEnvironment.DefaultDomain}/");
            Output<string> mcpResource = Output.Format(
                $"https://{names.Mcp}.{containerEnvironment.DefaultDomain}/mcp");
                        ...
                        new ContainerArgs
                        {
                            ...
                            [
                                SecretEnvironmentVariable(
                                    DatabaseConfigurationConstants.ConnectionStringEnvironmentVariable,
                                    AzureDeploymentConstants.AdministratorConnectionStringSecret)
                            ],
                    ...
                    [
                        new ContainerArgs
                        {
                            ...
                            [
                                ...
                                SecretEnvironmentVariable(
                                    ApiConfigurationConstants.ConnectionStringEnvironmentVariable,
                                    AzureDeploymentConstants.AdministratorConnectionStringSecret),
                                SecretEnvironmentVariable(
                                    ApiConfigurationConstants.ApiKeyEnvironmentVariable,
                                    AzureDeploymentConstants.ApiKeySecret),
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .ApplicationInsightsConnectionStringEnvironmentVariable,
                                    Value = insights.ConnectionString
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants.BlobProviderEnvironmentVariable, Value = "Azure"
                                },
                                new EnvironmentVarArgs
                                {
            ...
            ContainerApp mcp = new(names.Mcp, new ContainerAppArgs
            {
                ContainerAppName = names.Mcp,
                ResourceGroupName = resourceGroup.Name,
                Location = settings.Location,
                EnvironmentId = containerEnvironment.Id,
                Identity = UserAssignedIdentity(mcpIdentity.Id),
                Configuration = new Pulumi.AzureNative.App.Inputs.ConfigurationArgs
                {
                    ActiveRevisionsMode = ActiveRevisionsMode.Single,
                    Ingress = new IngressArgs
                    ...
                    Registries = [RegistryCredentials(registry, mcpIdentity)],
                    Secrets =
                    [
                        KeyVaultSecretReference(
                            AzureDeploymentConstants
                                .AdministratorConnectionStringSecret,
                            administratorConnectionStringSecret,
                            mcpIdentity),
                        KeyVaultSecretReference(
                            AzureDeploymentConstants.McpEntraClientSecret,
                            mcpEntraClientSecret,
                            mcpIdentity),
                        KeyVaultSecretReference(
                            AzureDeploymentConstants
                                .McpSigningCertificateSecret,
                            mcpSigningCertificate,
                            mcpIdentity),
                        KeyVaultSecretReference(
                            AzureDeploymentConstants
                                .McpEncryptionCertificateSecret,
                            mcpEncryptionCertificate,
                            mcpIdentity)
                    ]
                },
                Template = new TemplateArgs
                    ...
                    [
                        new ContainerArgs
                        {
                            ...
                            [
                                ...
                                {
                                    Name = AzureDeploymentConstants
                                        .AspNetCoreHttpPortsEnvironmentVariable,
                                    Value = "8080"
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .AspNetCoreForwardedHeadersEnvironmentVariable,
                                    Value = bool.TrueString
                                },
                                SecretEnvironmentVariable(
                                    DatabaseConfigurationConstants
                                ...
                                {
                                    Name = AzureDeploymentConstants
                                        .BlobProviderEnvironmentVariable,
                                    Value = "Azure"
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .BlobContainerUriEnvironmentVariable,
                                    Value = blobContainerUri
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .McpIssuerEnvironmentVariable,
                                    Value = mcpIssuer
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .McpResourceEnvironmentVariable,
                                    Value = mcpResource
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .McpEntraAuthorityEnvironmentVariable,
                                    Value = settings.McpEntraAuthority!
                                },
                                new EnvironmentVarArgs
                                {
                                    Name = AzureDeploymentConstants
                                        .McpEntraClientIdEnvironmentVariable,
                                    Value = settings.McpEntraClientId!
                                },
                                SecretEnvironmentVariable(
                                    AzureDeploymentConstants
                                        .McpEntraClientSecretEnvironmentVariable,
                                    AzureDeploymentConstants
                                        .McpEntraClientSecret),
                                SecretEnvironmentVariable(
                                    AzureDeploymentConstants
                                        .McpSigningCertificateEnvironmentVariable,
                                    AzureDeploymentConstants
                                        .McpSigningCertificateSecret),
                                SecretEnvironmentVariable(
                                    AzureDeploymentConstants
                                        .McpEncryptionCertificateEnvironmentVariable,
                                    AzureDeploymentConstants
                                        .McpEncryptionCertificateSecret)
                            ],
            ...
            RoleAssignment deploymentVaultAdministrator)
        {
            return new Secret(name,
                new SecretArgs
                {
                    ...
                    Tags = tags
                }, new CustomResourceOptions { DependsOn = [deploymentVaultAdministrator] });
        }
        ...
        {
            return new Pulumi.AzureNative.App.Inputs.SecretArgs
            {
                Name = name,
        ...
        {
            return new EnvironmentVarArgs { Name = name, SecretRef = secretName };
        }
        ...
        private static RegistryCredentialsArgs RegistryCredentials(
            ...
            UserAssignedIdentity identity)
        {
            return new RegistryCredentialsArgs { Server = registry.LoginServer, Identity = identity.Id };
        }

        private static JobConfigurationManualTriggerConfigArgs SingleReplicaManualTrigger()
        {
            return new JobConfigurationManualTriggerConfigArgs { Parallelism = 1, ReplicaCompletionCount = 1 };
        }

        private static ContainerResourcesArgs DatabaseJobResources()
        {
            return new ContainerResourcesArgs { Cpu = 0.5, Memory = "1Gi" };
        }
            ...
            ContainerApp? worker = null)
        {
            return new Dictionary<string, object?>
            {
                ...
                [AzureDeploymentConstants.ApiUrlOutput] = api is null
                    ? string.Empty
                    : api.Configuration.Apply(configuration =>
                        $"https://{configuration?.Ingress?.Fqdn ?? string.Empty}"),
                [AzureDeploymentConstants.McpUrlOutput] = mcp is null
                    ? string.Empty
                    : mcp.Configuration.Apply(configuration =>
                        $"https://{configuration?.Ingress?.Fqdn ?? string.Empty}/mcp"),
        ...
        {
            return new AppManagedIdentityArgs { Type = "UserAssigned", UserAssignedIdentities = [identityId] };
        }
        ...
        {
            return new RoleAssignment(CreateDeterministicGuid(seed),
                new RoleAssignmentArgs
                {
                    ...
                    Scope = scope
                }, new CustomResourceOptions { DependsOn = [dependency] });
        }
