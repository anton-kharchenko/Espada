...
{
    public static async Task RunAsync(string subscriptionId, string resourceGroupName, string jobName, CancellationToken cancellationToken)
    {
        ArmClient client = new(new DefaultAzureCredential(), subscriptionId);
        ResourceIdentifier identifier = ContainerAppJobResource.CreateResourceIdentifier(subscriptionId, resourceGroupName, jobName);
        ContainerAppJobResource job = client.GetContainerAppJobResource(identifier);

        ArmOperation<ContainerAppJobExecutionBase> operation = await job.StartAsync(WaitUntil.Completed, new ContainerAppJobExecutionTemplate(), cancellationToken);

        ContainerAppJobExecutionResource execution = client.GetContainerAppJobExecutionResource(new ResourceIdentifier(operation.Value.Id));
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            {
                throw new InvalidOperationException($"Container Apps job '{jobName}' finished with status '{status}'.");
            }
